    ...
    <div class="col-md-12">
      	<div class="box box-info">
            <div class="box-header with-border">
                ...
                <div class="box-tools">                 	
                    	<a href="<?php echo site_url('members-list'); ?>" class="btn btn-success btn-sm"><i class="fa fa-th-list" aria-hidden="true"></i>&nbsp;&nbsp;&nbsp; Users List</a>
                </div>
            </div>
          	...
          	<div class="box-body">
          		<div class="row clearfix">
                	...
                	<?php $readonly_class=""; 
						if(isset($edit_dtls))
						{
							$zone_ids_arr = explode(",", $edit_dtls['zone_ids']);
							$circle_ids_arr = explode(",", $edit_dtls['circle_ids']);
							$divisione_ids_arr = explode(",", $edit_dtls['division_ids']);
							$sub_division_ids_arr = explode(",", $edit_dtls['sub_division_ids']);
                    ...
                    <div class="col-md-4">
						...
						<div class="form-group">
                        	<select class="form-control" name="ddl_designation" id="ddl_designation" required onchange="get_zones(this.value)">
                    ...
                    <div class="col-md-4" id="dv_zone"></div>                    
                    <div class="col-md-4" id="dv_circle"></div>                   
                    <div class="col-md-4" id="dv_division"></div>                    
...
<?php if(isset($edit_dtls)){ ?>
	zone_ids = "<?php echo $edit_dtls["zone_ids"]; ?>";
	circle_ids = "<?php echo $edit_dtls["circle_ids"]; ?>";
	division_ids = "<?php echo $edit_dtls["division_ids"]; ?>";
...
function get_zones(des_id)
{
	...
	$("#dv_block").html('');
	$("#dv_panchayat").html('');
	$.post('<?php echo site_url('common_controller/get_zones') ?>', {des_id: des_id, zone_ids:zone_ids}, function(r) 
	{
		if(r != '')
		{
			$('#dv_zone').html(r);
...
function get_circles(zid)
{
	...
	$("#dv_block").html('');
	$("#dv_panchayat").html('');
	$.post('<?php echo site_url('common_controller/get_circles') ?>', {zid : zid, des_id: des_id, circle_ids:circle_ids}, function(r) 
	{
		if(r != '')
		{
			$('#dv_circle').html(r);
...
function get_divisions(cid)
{
	...
	$("#dv_block").html('');
	$("#dv_panchayat").html('');
	$.post('<?php echo site_url('common_controller/get_divisions') ?>', {cid : cid, des_id: des_id, division_ids:division_ids}, function(r) 
	{
		if(r != '')
		{
			$('#dv_division').html(r);
...
<?php if(isset($edit_dtls)){?>
	...
	get_zones('<?php echo $edit_dtls["designation_id"]; ?>');
	
	<?php if($edit_dtls["designation_id"] >= 6){?>
	get_circles('<?php echo $edit_dtls["zone_ids"]; ?>');
	<?php }if($edit_dtls["designation_id"] >= 7){?>
	get_divisions('<?php echo $edit_dtls["circle_ids"]; ?>');
